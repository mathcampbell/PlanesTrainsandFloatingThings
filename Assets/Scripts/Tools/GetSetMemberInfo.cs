...
namespace Tools
{
	...
	public class GetSetMemberInfo
	{
		...
		public TData GetValue<TData>(object instance)
		{
			if (isField)
			{
			...
			{
				return (TData) property.GetValue(instance);
			}
		}
		...
		public TData GetValueClass<TData>(object instance) where TData : class
		{
			if (isField)
			{
			...
			{
				return property.GetValue(instance) as TData;
			}
		}
