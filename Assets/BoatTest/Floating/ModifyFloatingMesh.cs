...
// Will cut the floating mesh where the water is
public class ModifyFloatingMesh
{
	...
	private Vector3[] boatVerticesGlobal;
	...
	public List<FloatingTriangleData> underWaterTriangleData = new List<FloatingTriangleData>();



	public ModifyFloatingMesh(GameObject boatObj)
	{
		// Get the transform
	...
	private void AddTriangles()
	{
		...
		while (i < boatTriangles.Length)
		{
			...
			if (vertexData[0].distance < 0f && vertexData[1].distance < 0f && vertexData[2].distance < 0f)
			{
				...
				// Save the triangle
				underWaterTriangleData.Add(new FloatingTriangleData(p1, p2, p3));
			}
	...
	private void AddTrianglesOneAboveWater(List<VertexData> vertexData)
	{
		...
		underWaterTriangleData.Add(new FloatingTriangleData(M, I_M, I_L));
		underWaterTriangleData.Add(new FloatingTriangleData(M, I_L, L));
	}
	...
	private void AddTrianglesTwoAboveWater(List<VertexData> vertexData)
	{
		...
		underWaterTriangleData.Add(new FloatingTriangleData(L, J_H, J_M));
	}
	...
	public void DisplayMesh(Mesh mesh, string name, List<FloatingTriangleData> triangesData)
	{
