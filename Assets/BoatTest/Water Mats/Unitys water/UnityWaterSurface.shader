...
Shader "Custom/Water/UnityWaterWithWave"
{
	Properties
	{
	...
	SubShader
	{
		...
		float getHeightFromNoise(float2 uv)
		{
			...
			//-1->1
			float oldRange = (1.0 - 0.0);
			float newRange = (1.0 - (-1.0));
			float newValue = (((height - 0) * newRange) / oldRange) + (-1.0);
		...
		void vert(inout appdata_full i, out Input o)
		{
			...
			float4 positionAlongTangent = float4(getWavePos(newTangentPos.xyz), newTangentPos.w);
